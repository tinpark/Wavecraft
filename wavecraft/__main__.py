...
def main():
    ...
    parser.add_argument("operation", type=str, choices=["segment", "extract", "proxim", "onset", "beat", "decomp", "concat", "filter", "norm", "fade", "trim", "split", "pan", "rename", "wmeta"], 
    ...
    # Concatenation
    concat_group = parser.add_argument_group(title='Concatenation - concatenates multiple audio files into a single file', description='operation -> concat')
    concat_group.add_argument("-ctg", "--concat-target", default=None, help='Target file path')
    concat_group.add_argument("-ws", "--win-size", type=int, default=2048, help='Window size. Default is 2048.', required=False, metavar='')
    concat_group.add_argument("-nmfi", "--nmf-iterations", type=int, default=12, help='NMF iterations. Default is 10.', required=False, metavar='')
    concat_group.add_argument("-psr", "--pitch-shift-range", type=int, default=24, help='Pitch shift range. Default is 6.', required=False, metavar='')
    concat_group.add_argument("-fw", "--filter-width", type=int, default=4, help='Filter width. Default is 3.', required=False, metavar='')
    concat_group.add_argument("-pd", "--poly-degree", type=int, default=1, help='Polyphony degree. Default is 3.', required=False, metavar='')
    concat_group.add_argument("-hl", "--half-length", type=int, default=6, help='Activation half length. Default is 1.', required=False, metavar='')
    ...
    normalization_group.add_argument("-nm", "--normalisation-mode", type=str, default="peak", choices=["peak", "rms", "loudness"],
                        help="Normalisation mode; choices are : peak, rms, loudness. Default is 'peak'.", required=False, metavar='')
